...
IN THE FILE SUBMITTED TWO DAYS AGO, I REALIZED I HAD ACCIDENTALLY DELETED 
"FROM MPATH IMPORT*"
AND
"FROM MATH IMPORT*"

I HOPE I DONT GET A LATE PENALTY ON THAT PLEASE, AS IT WAS AN ACCIDENTAL DELETION
    ...
    for i in range(len(theList)):
        for j in range(i + 1, len(theList)):
            ...
            if equal(theList[i], theList[j]) == True: #here's where I compare them
                ...
                print(number, ')', theList[i], ' = ', theList[j])
                print()
                number += 1
